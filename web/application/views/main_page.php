...
<div id="app">
            ...
            <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
                ...
                <li class="nav-item">
                    <?  if (User_model::is_logged()) {?>
                        ...
                        <button @click="getHistory" type="button" class="btn btn-success my-2 my-sm-0" data-toggle="modal"
                ...
                <div class="modal-body">
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text" v-for="comment in postComments">
                                ...
                                <a style="cursor:pointer;" role="button" @click="selectForReply(comment.id, comment.text)">
                                    REPLY
                                </a>
                                <span v-for="reply in comment.replies">
                      <br>
                      --------------
                  {{reply.user.personaname + ' - '}}
                      <small class="text-muted">{{reply.text}}</small>
                  <a role="button" @click="addLike('comment', reply.id)">
                      <svg class="bi bi-heart-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" clip-rule="evenodd"/>
                      </svg>
                      {{ reply.likes ?? 0 }}
                  </a>
                  <a style="cursor: pointer" role="button" @click="selectForReply(reply.id, reply.text)">
                      REPLY
                  </a>
                  </span>
                            </p>
                            <form class="form-inline">
                                <p v-if="reply_to">{{reply_to}}</p>
                                <div class="form-group">
    ...
    <div class="modal fade" id="historyModal" tabindex="-1" role="dialog" aria-labelledby="historyModal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">historyModal</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false">Totals</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" id="booster-tab" data-toggle="tab" href="#booster" role="tab" aria-controls="booster" aria-selected="false">Booster Pack History</a>
                        </li>
                    </ul>
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                            <p v-for="record in user_history"> {{record.time_created}} {{record.action == 1 ? 'refill' : 'withdraw'}} {{record.object}} {{record.amount}} </p>
                        </div>
                        <div v-if="totals" class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                            <p>Like Balance {{totals.likes_balance}}</p>
                            <p>Wallet balance {{totals.wallet_balance}}</p>
                            <p>Total refilled {{totals.wallet_total_refilled}}</p>
                            <p>total withdrawn {{totals.wallet_total_withdrawn}}</p>
                        </div>
                        <div v-if="booster_history" class="tab-pane fade" id="booster" role="tabpanel" aria-labelledby="booster-tab">
                            <p v-for="history in booster_history"> {{history.time_created}} Price: {{history.boosterpack_price}} Likes received: {{history.received}} </p>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
